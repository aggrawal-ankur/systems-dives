...
/* HEAP_MIN_SIZE and HEAP_MAX_SIZE limit the size of mmap()ed heaps
   ...
   - The maximum size must be a power of two, for fast determination 
     of which heap belongs to a chunk.
   - It should be much larger than the mmap threshold, so that 
     requests with a size just below that threshold can be fulfilled 
     without creating too many heaps. */

/* HEAP_MAX_SIZE should be larger than the huge page size, 
   otherwise heaps will use not huge pages. It is a constant 
   so arena_for_chunk() is efficient. */
...
/* A heap is a single contiguous memory region holding (coalesce-able)
...
extern int sanity_check_heap_info_alignment[
  (sizeof(heap_info) + 2 * SIZE_SZ) % MALLOC_ALIGNMENT 
  ? -1 
  : 1
];
...
/* arena_get() acquires an arena and locks the corresponding mutex.
   - First, try the one last locked successfully by this thread. (This
     is the common case and handled with a macro for speed.)
   - Then, loop once over the circularly linked list of arenas. If no 
     arena is readily available, create a new one. In this latter case, 
     `size` is just a hint as to how much memory will be required 
     immediately in the new arena. */
...
/* The following three functions are called around fork from a
   ...
   We do not use the general fork handler mechanism to make 
   sure that our handlers are the last ones being called, so 
   that other fork handlers can use the malloc subsystem. */
...
static void TUNABLE_CALLBACK (__name) (tunable_val_t *valp)				\
{									      \
...
#ifdef USE_MTAG
  if (
    (TUNABLE_GET_FULL(glibc, mem, tagging, int32_t, NULL) & 1) != 0
  ){
...
# endif
  ...
  if (mp_.hp_pagesize > 0 && mp_.hp_pagesize <= heap_max_size()){
    /* Force mmap for main arena instead of sbrk, so MAP_HUGETLB 
       is always tried. Also, tune the mmap threshold, so allocation 
...
/* If consecutive mmap (0, HEAP_MAX_SIZE << 1, ...) calls return decreasing
   ...
   address space.
   - In that case remember the second HEAP_MAX_SIZE part aligned to 
     HEAP_MAX_SIZE from last mmap (0, HEAP_MAX_SIZE << 1, ...) call 
     (if it is already aligned) and try to reuse it next time.
   - We need no locking for it, as kernel ensures the atomicity for us,
     the worst case we'll call mmap (addr, HEAP_MAX_SIZE, ...) for some 
     value of addr in multiple threads, but only one will succeed. */

static char *aligned_heap_area;

/* Create a new heap. size is automatically rounded up to 
...
static heap_info* alloc_new_heap(
  ...
  if (p2 == MAP_FAILED){
    ...
    if (p1 != MAP_FAILED){
      p2 = (
        (char*) (((uintptr_t)p1 + (max_size-1)) & ~(max_size-1))
      );
...
static int grow_heap(heap_info *h, long diff)
{
  ...
  /* mprotect preserves MADV_HUGEPAGE semantics - this means, 
     if the old region was marked with MADV_HUGEPAGE, the 
...
static int shrink_heap(heap_info *h, long diff)
{
  ...
  /* Try to re-map the extra heap space freshly to save memory, 
     and make it inaccessible. See malloc-sysdep.h to know 
     when this is true. */
...
static void detach_arena(mstate replaced_arena)
{
  if (replaced_arena != NULL){
    ...
    /* The current implementation only detaches from main_arena in
       ...
       This means that it is likely not beneficial to put the arena 
       on free_list even if the reference count reaches zero. */
...
static mstate _int_new_arena(size_t size)
{
  ...
  if (!h){
    /* Maybe size is too large to fit in a single heap.
       So, just try to create a minimally-sized arena and 
       let _int_malloc() attempt to deal with the large 
       request via mmap_chunk(). */
  ...
  /* FIXME: The barrier is an attempt to synchronize with read 
     access in reused_arena, which does not acquire list_lock 
  ...
  /* Lock this arena. NB: Another thread may have been attached 
     to this arena because the arena is now accessible from the
     main_arena.next list and could have been picked by reused_arena.
     This can only happen for the last arena created (before the arena
     limit is reached). At this point, some arena has to be attached
     to two threads. We could acquire the arena lock before list_lock
     to make it less likely that reused_arena picks this new arena,
     but this could result in a deadlock with __malloc_fork_lock_parent. */
  ...
  if (a == NULL){
    ...
    repeat:;
      ...
      /* NB: the following depends on the fact that (size_t)0 - 1 is a
         very large number and that the underflow is OK.
         - If arena_max is set the value of arena_test is irrelevant.
         - If arena_test is set but narenas is not yet larger or equal 
           to arena_test narenas_limit is 0.

        There is no possibility for narenas to be too big for the test 
        to always fail since there is not enough address space to 
        create that many arenas. */
...
/* If we don't have the main arena, then maybe the failure is 
   due to running out of mmapped areas, so we can try allocating 
   on the main arena.
   Otherwise, it is likely that sbrk() has failed and there is 
